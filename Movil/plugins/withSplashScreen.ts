...
const withCustomStylesXml: ConfigPlugin = (config) =>
  withAndroidStyles(config, async (modConfig) => {
    ...
    const splashScreenStyle = modConfig.modResults.resources.style?.find(
      (style) => style.$.name === "Theme.App.SplashScreen"
    )

    if (splashScreenStyle) {
      // Change the parent to AppTheme
      splashScreenStyle.$.parent = "AppTheme"
    } else {
      // If the style doesn't exist, throw it as an error or create it (optional)
      throw new Error("Theme.App.SplashScreen not found in styles.xml")
    }
