...
interface CreateFundManagementCardProps {
  navigation: any; 
}

export const CreateFundManagementCard: FC<CreateFundManagementCardProps> = ({ navigation }) => {
  return (
      <View style={$card}>
      <View style={$header}>
        <Text style={$title} text="Crear Fondos"/>
      </View>
      <Text style={$description}>
        Crea los fondos disponibles para asignar en la solicitudes de Recursos
      </Text>
      <Button 
        text="Crear nuevo Fondo de Recursos" 
        style={$pinkButton}
        textStyle={$pinkButtonText}
        pressedStyle={$pinkButtonPressed}
        onPress={() => {
          navigation.navigate('CreateFundManagement')
        }}
      />
    </View>
