...
interface EditIncentiveSelectedFilesProps {
  files: { id: number; fileName: string }[]; 
  incentiveRequestId: number;
}

const EditIncentiveSelectedFiles: React.FC<EditIncentiveSelectedFilesProps> = ({
  files: initialFiles,
  incentiveRequestId,
}) => {
  ...
  const handleDelete = async (fileId: number) => {
    try {
      ...
      const response = await api.deleteScholarshipFiles(incentiveRequestId, fileId, accessToken);
...
export default EditIncentiveSelectedFiles;
