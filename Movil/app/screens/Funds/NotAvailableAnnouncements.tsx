...
interface NotAvailableAnnouncementsScreenProps {
  navigation: any;
}

export const NotAvailableAnnouncements: FC<NotAvailableAnnouncementsScreenProps> = ({ navigation }) => {
  ...
  useHeader({
    title: "Mis postulaciones a Anuncios",
    leftIcon: "back",
  ...
  const fetchAnnouncements = useCallback(async () => {
    try {
      ...
      if ("ok" in response) {
        const { registeredAnnouncements } = response.data;
        if (registeredAnnouncements && Array.isArray(registeredAnnouncements)) {
          setAnnouncements(registeredAnnouncements);
        } else {
  ...
  const renderAnnouncement = ({ item }: { item: Announcement }) => (
    <View style={$announcementCard}>
      ...
      <Text style={$description}>{item.motivationMessage}</Text>
    </View>
  );
